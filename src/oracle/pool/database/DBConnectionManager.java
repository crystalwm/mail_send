...
public class DBConnectionManager {
 ...
 /**
  * 获得一个空闲的连接.如果没有空闲连接且已有连接数小于最大连接数
  * 则创建并返回新连接
  *
  * @param name 在属性文件中定义的连接池名字
 ...
 /**
  * 获得一个空闲的连接.若没有空闲的连接,且已有连接数小于最大连接数限制,
  ...
  */
  private void init() {
  InputStream is = getClass().getResourceAsStream("/DBConfig.properties");	//读取db.properties文件中的属性配置
 ...
 /**
  * 此内部类定义了一个连接池.
  * 它能够根据要求创建新连接,直到预定的最大连接数为止.
  * 在返回连接给应用程序之前,它能够验证连接的有效性.
  * 
  */
 ...
 /**
  * 从连接池获得一个空闲的连接.
  * 如没有空闲的连接且当前连接数小于最大连接数限制,
  * 则创建新连接.
  * 如原来登记为空闲的连接不再有效,则从向量Vector中删除,
  * 然后递归调用以尝试新的空闲连接.
  */
